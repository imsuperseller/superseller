...
class BenAgentActivator {
  ...
  async healthCheck() {
    try {
      const response = await axios.get(`${this.benConfig.n8nUrl}/api/v1/health`, {
  ...
  async activateAllAgents() {
    console.log('🚀 ACTIVATING BEN GINATI AGENTS');
    console.log('================================');
    ...
    for (const agent of this.agents) {
      ...
      const workflow = workflows.find(w => 
        ...
        w.name.includes(agent.name.split(' ')[0]) // Match by first word
      );
...
const activator = new BenAgentActivator();

async function main() {
  console.log('🎯 BEN GINATI - AGENT ACTIVATION');
  console.log('==================================');
