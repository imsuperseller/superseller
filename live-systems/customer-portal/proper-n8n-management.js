...
class ProperN8NManager {
  constructor() {
    ...
    // Cloud Configuration - Limited API Access (Tax4US LLC)
    this.cloudConfig = {
    ...
    // Shelly Customer Cloud Configuration (Secondary Customer)
    this.shellyConfig = {
  ...
  async manageVPSInstance() {
    ...
    for (const workflowName of this.criticalWorkflows) {
      ...
      const result = await this.callMCPServer('execute_workflow', { workflowId: workflow.id });
      ...
      if (result) {
        ...
        // Fallback to direct API activation
        console.log(`🔄 MCP failed, trying direct API activation: ${workflow.name}`);
        const directResult = await this.activateVPSWorkflow(workflow.id);
        if (directResult) {
          console.log(`✅ Successfully activated via direct API: ${workflow.name}`);
          activationResults.push({
            name: workflow.name,
            status: 'activated_via_direct_api',
            success: true,
            instance: 'vps'
          });
        } else {
