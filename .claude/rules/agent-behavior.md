...
- **Rate table**: See `INFRA_SSOT.md` §5b or `.claude/skills/videoforge-pipeline/SKILL.md`.
...
2. **Which product does it fit?** VideoForge, Winner Studio, AgentForge, FrontDesk — map the discovery to an existing product or note it as a new product seed.
...
## Task Sizing (Spec-Driven Planning)

| Size | Scope | Action |
|------|-------|--------|
| **Small** | 1-2 files | Execute immediately, single commit |
| **Medium** | 3-5 files | SPEC → PLAN → atomic commits (see `spec-driven-dev` skill) |
| **Large** | 6+ files or new system | Full SPEC → PLAN with subagent isolation → atomic commits. For new projects, B.L.A.S.T. Blueprint = the SPEC. |

### Spec-Driven Workflow (Medium + Large)

Before writing code on any 3+ file task:
1. **SPEC** (30s): Write requirements + acceptance criteria inline in response
2. **PLAN**: Decompose into atomic tasks, each with a commit message
3. **EXECUTE**: One task at a time → verify → commit
4. **VERIFY**: Check acceptance criteria from SPEC

Full workflow: `.claude/skills/spec-driven-dev/SKILL.md`

### Atomic Commit Discipline

- Each logical unit of work = one commit. Not one giant commit at the end.
- Use conventional prefixes: `feat:`, `fix:`, `refactor:`, `test:`, `docs:`, `chore:`
- Each commit should be independently revertable without breaking other work.
- Push after final commit (not after every commit).

### Codebase Mapping

Run `npx tsx tools/map-codebase.ts` to auto-refresh `REPO_MAP.md` before Large tasks. Ensures the codebase index matches reality.

---

## NotebookLM Sync (Optional Reference)

NotebookLM notebooks are a reference resource, not a mandatory sync target. Update when convenient, not on every task.

### Notebook-to-Product Map (for reference)
| Product Changed | Notebook |
|-----------------|-----------|
| VideoForge / video pipeline | 0baf5f36 (Zillow-to-Video) |
...
| Agent behavior / methodology | 1dc7ce26 (BLAST) |
