...
## When to Use
Use when working on AgentForge research pipeline, adding/modifying pipeline stages, credit integration for reports, deliverable generation, or client intake flows. Not for video pipeline (use videoforge-pipeline), UI design (use ui-ux-pro-max), or billing logic (use customer-journey for PayPal billing and credit management).
...
4. **Use BullMQ for pipeline orchestration.** Same pattern as VideoForge: queue job → worker processes stages sequentially → poll for progress. Never run multi-minute pipelines synchronously in API routes.
...
- `apps/worker/src/queue/workers/video-pipeline.worker.ts` — VideoForge pipeline pattern to follow
- customer-journey skill — Credit check/deduct patterns
