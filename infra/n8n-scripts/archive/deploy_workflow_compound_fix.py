...
# 1. Update PrepareUpdateSuccess to be extremely robust
for node in wf['nodes']:
    if node['name'] == 'PrepareUpdateSuccess':
        # Now merging everything from StoreSubmission to ensure matching columns are present
...
return {
    json: {
        ...submission, // This includes address, email, submissionDate, etc.
...
# 2. Update UpdateSuccess to use address + submissionDate for matching
for node in wf['nodes']:
    if node['name'] == 'UpdateSuccess':
        node['parameters']['columns']['matchingColumns'] = ["address", "submissionDate"]
...
print(f"Deploying compound match fix to {WORKFLOW_ID}...")
...
if response.status_code == 200:
    print("Workflow updated successfully with compound match!")
else:
