...
# 1. Update FinalListBuilder1 to handle 0-video case gracefully
...
# 2. Update UpdateSuccess to handle failure case status
for node in wf['nodes']:
    if node['name'] == 'UpdateSuccess':
        if 'columns' in node['parameters'] and 'value' in node['parameters']['columns']:
            # status expression: if video_url exists in either input, it's completed, otherwise failed
            node['parameters']['columns']['value']['status'] = "={{ ($json.video_url || ($node['MergeVideosintoTour1'].json && $node['MergeVideosintoTour1'].json.video_url)) ? 'completed' : 'failed_no_videos' }}"
            node['parameters']['columns']['value']['finalTourUrl'] = "={{ $json.video_url || ($node['MergeVideosintoTour1'].json ? $node['MergeVideosintoTour1'].json.video_url : '') }}"
...
payload = {
    ...
    "settings": {"executionOrder": "v1"} # Keep minimal settings as verified
}
...
print(f"Deploying final robustness update to {WORKFLOW_ID}...")
response = requests.put(f"{INSTANCE_URL}/workflows/{WORKFLOW_ID}", json=payload, headers=headers)

if response.status_code == 200:
    print("Workflow updated successfully with 0-video handling!")
else:
