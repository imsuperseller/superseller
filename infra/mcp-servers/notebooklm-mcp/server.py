import os
import sys
# Dependencies from Homebrew and UV cache
sys.path.append("/opt/homebrew/lib/python3.11/site-packages")
sys.path.append("/Users/shaifriedman/.cache/uv/archive-v0/LlYAH2GOPFDL4ZqHSovRG/lib/python3.12/site-packages")
...
import api_client
import constants
__version__ = "1.0.0" # Placeholder or extract from file
...
# Global state
_client: api_client.NotebookLMClient | None = None
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                # Fallback error handling: return the error as a string instead of letting it crash the server
                # and pollute stdout with a traceback.
                err_msg = f"Error in {tool_name}: {str(e)}"
                mcp_logger.error(err_msg)
                return err_msg
...
def get_client() -> api_client.NotebookLMClient:
    ...
    if _client is None:
        ...
        if cookie_header:
            ...
            cookies = api_client.extract_cookies_from_chrome_export(cookie_header)
        else:
        ...
        try:
            _client = api_client.NotebookLMClient(
                cookies=cookies,
                csrf_token=csrf_token,
                session_id=session_id,
            )
        except ValueError as e:
            # Re-raise as a more generic exception that we'll catch in the tool functions
            # to ensure it's returned as a JSON error response, not a printed traceback.
            raise RuntimeError(f"NotebookLM API Client initialization failed: {e}")
    return _client
    ...
    try:
        ...
        if result and isinstance(result, list) and len(result) > 5:
            ...
            if isinstance(metadata, list):
                if len(metadata) > 5:
                    modified_at = api_client.parse_timestamp(metadata[5])
                if len(metadata) > 8:
                    created_at = api_client.parse_timestamp(metadata[8])
...
Examples:
  ...
  notebooklm-mcp --query-timeout 180          # Set query timeout to 180 seconds
    ...
    if args.debug:
        ...
        # pass # print("Debug logging: ENABLED (MCP tool calls + NotebookLM API requests/responses)")
    
    if args.transport == "http":
        # pass # print(f"Starting NotebookLM MCP server (HTTP) on http://{args.host}:{args.port}{args.path}")
        # pass # print(f"Health check: http://{args.host}:{args.port}/health")
        # if args.stateless:
        #     pass # print("Stateless mode: ENABLED (suitable for horizontal scaling)")
        mcp.run(
            transport="http",
            host=args.host,
            port=args.port,
        ...
        # pass # print(f"Starting NotebookLM MCP server (SSE) on http://{args.host}:{args.port}/sse")
        # pass # print(f"Health check: http://{args.host}:{args.port}/health")
        # if args.stateless:
        #     pass # print("Stateless mode: ENABLED (suitable for horizontal scaling)")
        mcp.run(
