        ...
        for (let i = 0; i < toCreate; i++) {
            try {
                ...
                if (clientId === 'uad') {
                    ...
                    productName = config.productName;
                    price = MISSPARTY_PRICE; // Fixed $49.99 — NO jitter
                    uniqueHash = config.uniqueHash;
                    configData = config.configData;
                ...
                if (!images) {
                    console.error(`[REPLENISH] ${clientId.toUpperCase()} [${i + 1}/${toCreate}] Skipping — image generation failed (no duplicate static fallback)`);
                    continue;
                }
                ...
                let listingDescription = clientId === 'uad'
                    ? `${config.configData?.size || ''} ${productName} in ${config.configData?.color || ''} ${config.configData?.design || ''} finish. Includes delivery and setup. Text ${activePhone || product.phoneRotation[0]} for details.`
                    : `White Bounce House available for 24hr rental. ${MISSPARTY_DELIVERY} Text ${product.phoneRotation[0]} to reserve.`;
                ...
                // Never reuse same image — prevents Facebook "duplicate listing" flags
                const dupImage = await pool.query(
                    `SELECT 1 FROM fb_listings WHERE client_id = $1 AND image_url = $2 AND status IN ('queued','posted') LIMIT 1`,
                    [clientId, images.imageUrl]
                );
                if (dupImage.rows.length > 0) {
                    console.error(`[REPLENISH] ${clientId.toUpperCase()} [${i + 1}/${toCreate}] Skip — image_url already used`);
                    continue;
                }

                // Avoid duplicate titles (same title = duplicate listing warning)
                const dupTitle = await pool.query(
                    `SELECT 1 FROM fb_listings WHERE client_id = $1 AND listing_title = $2 AND created_at > NOW() - INTERVAL '14 days' LIMIT 1`,
                    [clientId, listingTitle]
                );
                if (dupTitle.rows.length > 0) {
                    listingTitle = `${listingTitle} — ${rotatedLocation}`;
                }
