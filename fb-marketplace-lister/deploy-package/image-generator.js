...
// Subtitles disabled — phone overlay removed to avoid FB Commerce Policy violations
...
async function generateListingImages(clientId, config, phoneNumber, opts = {}) {
    ...
    for (let i = 0; i < 3; i++) {
        ...
        // Skip phone overlay — Facebook flags images with burned-in phone numbers
        // as Commerce Policy violations (service ad detection via OCR).
        // Phone number goes in listing description only.
        ...
        console.log(`[IMG-GEN] [${clientId.toUpperCase()}] Image ${i + 1} ready (${size}KB) — clean, no overlay`);
    ...
    // Do NOT use same static image for every post (Facebook duplicate / user complaint).
    // If generation failed, return null so this listing is not queued — replenishment will retry later.
    if (!results.imageUrl) {
        console.warn(`[IMG-GEN] [${clientId.toUpperCase()}] All images failed — skipping listing (no static fallback)`);
        return null;
    }
