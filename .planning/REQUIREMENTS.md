...
- [x] **PROV-04**: System can generate dialogue/talking-head video via Veo 3.1 on kie.ai (/api/v1/veo/generate)
...
- [x] **QUAL-02**: Nightly aggregation job computes avg quality_score per model from content_entries.performanceScore
- [x] **QUAL-03**: Aggregation job updates ai_model_recommendations.quality_score only when sample_count >= 20
...
- [x] **QUAL-06**: Router uses Observatory quality_score (real feedback data) when selecting models, not just static scores
...
| QUAL-02 | Phase 09 | Complete |
| QUAL-03 | Phase 09 | Complete |
