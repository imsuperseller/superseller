#!/usr/bin/env node
/**
 * Regenerate Yaron's VideoForge with ALL fixes:
 * - 1920x1080 forced output
 * - Fresh Suno music
...
const NEG = "cartoon, anime, CGI, duplicate person, two people, clone, extra person, different face, wrong furniture, misplaced objects, items on floor, floating objects, blurry, low quality, morphing, wall penetration";

// V2 prompts: Longer durations, smoother transitions, proper furniture placement
const CLIPS_V2 = [
  {
  ...
  },
  {
    ...
    negative: `${NEG}, kitchen items, bathroom items, bedroom furniture`,
  },
  {
    ...
    negative: `${NEG}, unmade bed, messy clothes, kitchen items, living room items`,
  },
  {
  ...
  },
  {
    ...
    negative: `${NEG}, unmade bed, messy clothes, bathroom items, kitchen items`,
  },
  {
  ...
  },
  {
    ...
    negative: `${NEG}, unmade bed, bathroom items, kitchen items, bed furniture`,
  },
  {
    ...
    negative: `${NEG}, toilet seat up, dirty towels, soap scum, chairs, kitchen items, bedroom items`,
  },
  {
    ...
    negative: `${NEG}, rain, cloudy sky, dead plants, brown lawn, trash, indoor furniture`,
  },
];
  ...
  try {
    ...
    const jobResult = await pool.query(
      `INSERT INTO video_jobs (id, listing_id, user_id, status, model_preference, tour_sequence, total_clips, completed_clips, progress_percent, created_at, updated_at)
       ...
       RETURNING id`,
      [LISTING_ID, YARON_USER_ID, JSON.stringify(TOUR_SEQUENCE), CLIPS_V2.length]
    );
    const jobId = jobResult.rows[0].id;
    console.log(`✅ Created video_job V2: ${jobId}`);

    // 2. Pre-seed clips with V2 prompts
    for (const c of CLIPS_V2) {
      await pool.query(
    ...
    console.log(`✅ Seeded ${CLIPS_V2.length} clips with improved prompts`);
