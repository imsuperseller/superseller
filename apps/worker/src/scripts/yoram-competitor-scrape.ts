/**
 * Yoram Friedman Insurance — Competitor Ad Research (Two-Pass AI Pipeline)
 *
 ...
 *
 * Run: cd apps/worker && npx tsx src/scripts/yoram-competitor-scrape.ts
 */
...
const TENANT_ID = "yoram-friedman-insurance";

// Israeli insurance search terms — Hebrew + English variations
const SEARCH_TERMS = [
  "ביטוח חיים ישראל",           // Life insurance Israel
  "סוכן ביטוח",                  // Insurance agent
  "ביטוח פנסיוני",               // Pension insurance
  "משכנתא ביטוח",                // Mortgage insurance
  "ביטוח מנהלים",                // Executive insurance (Bituach Menahalim)
  "קרן פנסיה",                   // Pension fund
  "ביטוח בריאות פרטי",           // Private health insurance
  "חיסכון פנסיוני",              // Pension savings
  "ביטוח חיים וחיסכון",          // Life insurance and savings
  "יועץ פנסיוני",                // Pension advisor
];

// No WhatsApp group yet for Yoram — will just print and store
const YORAM_WHATSAPP_GROUP = process.env.YORAM_WHATSAPP_GROUP || "";
...
async function pass1Extract(ad: ScrapedAd): Promise<Record<string, unknown> | null> {
  ...
  contentParts.push({
    type: "text",
    text: `Analyze this competitor ad from an Israeli insurance/pension company.
The ad may be in Hebrew. Analyze regardless of language.
...
async function pass2Replicate(ad: ScrapedAd, pass1: Record<string, unknown>): Promise<string | null> {
  const prompt = `You are a senior insurance marketing strategist working for Yoram Friedman (יורם פרידמן), an Israeli insurance agent.

YORAM FRIEDMAN CONTEXT:
- Licensed insurance agent (#68423), active since 1984 — over 40 years of experience
- Specializes in: life insurance (ביטוח חיים), pension planning (פנסיה), mortgages (משכנתאות), savings (חיסכון), executive insurance (ביטוח מנהלים)
- Brand: trusted, veteran, personal service — "the agent who's been there for 40 years"
- USP: Deep industry knowledge spanning 4 decades, personal relationship with each client, independent agent (not tied to one company — can compare all options)
- Target audience: Israeli professionals 30-55 concerned about retirement, young families, small business owners needing executive insurance
- Website: yoramfriedman.co.il — dark premium design with gold accents
- Lead magnet: Free portfolio review (בדיקת תיק ללא עלות)
- Mascot/brand: Professional but warm, father-figure trust, gold and navy colors
...
Write a specific, actionable replication brief (3-5 sentences) for Yoram Friedman. Include:
1. What EXACTLY makes this ad work — be specific about the psychological mechanism, not generic "great hook"
2. How Yoram should adapt this approach — specific ad angle, headline in Hebrew, visual concept, what to show
3. What to change/improve vs the competitor's version — leverage Yoram's 40-year track record, independent agent advantage, or personal touch

Be concrete. Reference Yoram by name. Include at least one Hebrew headline suggestion. Think about what insurance offer (pension check, portfolio review, savings comparison) would pair best with this ad style.`;
...
async function main() {
  console.log("=== Yoram Friedman Insurance — Competitor Ad Research ===\n");
  ...
  let summary = `*מחקר מתחרים — יורם פרידמן ביטוח*\n\n`;
  summary += `נמצאו ${provenAds.length} פרסומות מוכחות של מתחרים (${MIN_DAYS_RUNNING}+ ימים פעילות)\n\n`;
  ...
  for (let i = 0; i < topAds.length; i++) {
    ...
    summary += `${i + 1}. *${ad.pageName || "לא ידוע"}* — ${days} ימים${score}${product}\n`;
  ...
  if (YORAM_WHATSAPP_GROUP) {
    console.log("\nPhase 4: Sending to WhatsApp...");
    try {
      await sendText(YORAM_WHATSAPP_GROUP, summary);
