...
export function CustomerQuestions({ customerId, questions, onQuestionUpdate, onAddNote }: CustomerQuestionsProps) {
  ...
  const getStatusIcon = (status: Question['status']) => {
    switch (status) {
      ...
      case 'in-progress':
        return <Clock className="w-5 h-5 style={{ color: 'var(--superseller-blue)' }}" />;
      case 'blocked':
        return <AlertCircle className="w-5 h-5 style={{ color: 'var(--superseller-red)' }}" />;
      default:
                  ...
                  <div className="space-y-2">
                    {selectedQuestion.attachments.map((attachment, index) => (
                      <a
                        key={index}
                        href={attachment}
                        className="flex items-center space-x-2 style={{ color: 'var(--superseller-blue)' }} hover:style={{ color: 'var(--superseller-blue)' }}"
                      >
                        <span>📎</span>
