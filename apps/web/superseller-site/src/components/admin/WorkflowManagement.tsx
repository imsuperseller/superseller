...
export default function WorkflowManagement({ products = [] }: WorkflowManagementProps) {
  ...
  const openDebug = (workflowId: string, executionId?: string) => {
    if (executionId) {
      window.open(`https://n8n.superseller.agency/workflow/${workflowId}/executions/${executionId}`, '_blank');
    } else {
      window.open(`https://n8n.superseller.agency/workflow/${workflowId}`, '_blank');
    }
  };
