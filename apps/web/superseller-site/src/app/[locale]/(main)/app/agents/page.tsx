...
export default function AgentsPage() {
  ...
  const getStatusBadge = (status: string) => {
    ...
    switch (s) {
      ...
      case 'paused':
      case 'stopped':
        return <Badge variant="supersellerWarning">Paused</Badge>;
      case 'inactive':
      case 'error':
        return <Badge variant="supersellerError">Inactive</Badge>;
      default:
  ...
  const getTypeBadge = (serviceId: string) => {
    ...
    if (serviceId.includes('content') || serviceId.includes('blog')) return <Badge variant="supersellerPrimary">Content</Badge>;
    if (serviceId.includes('social') || serviceId.includes('linkedin')) return <Badge variant="supersellerSecondary">Social</Badge>;
    if (serviceId.includes('automation') || serviceId.includes('scraper')) return <Badge variant="supersellerNeon">Automation</Badge>;
    return <Badge variant="supersellerSecondary">Custom</Badge>;
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-superseller-text-primary">Agents</h1>
          <p className="text-superseller-text-secondary mt-1">Manage your active AI services</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <Card variant="supersellerNeon" className="superseller-card-neon">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-superseller-text-secondary">Total Agents</CardTitle>
            <Workflow className="h-4 w-4 text-superseller-cyan" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-superseller-text-primary">{agents.length}</div>
            <p className="text-xs text-superseller-text-tertiary">Deployed services</p>
          </CardContent>
        </Card>

        <Card variant="supersellerNeon" className="superseller-card-neon">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-superseller-text-secondary">Active</CardTitle>
            <Activity className="h-4 w-4 text-green-400" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-superseller-text-primary">
              {agents.filter(a => a.status === 'active' || a.status === 'running').length}
            </div>
            <p className="text-xs text-superseller-text-tertiary">Currently running</p>
          </CardContent>
        </Card>

        <Card variant="supersellerNeon" className="superseller-card-neon">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-superseller-text-secondary">Failures (7d)</CardTitle>
            <AlertTriangle className="h-4 w-4 text-superseller-red" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-superseller-text-primary">0</div>
            <p className="text-xs text-superseller-text-tertiary">System stable</p>
          </CardContent>
        </Card>

        <Card variant="supersellerNeon" className="superseller-card-neon">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-superseller-text-secondary">Scheduled</CardTitle>
            <Clock className="h-4 w-4 text-superseller-text-tertiary" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-superseller-text-primary">
              {agents.filter(a => a.parameters?.schedule).length}
            </div>
            <p className="text-xs text-superseller-text-tertiary">Auto-running</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card variant="supersellerNeon" className="superseller-card-neon">
        <CardHeader>
          <CardTitle className="flex items-center space-x-2 text-superseller-text-primary">
            <Filter className="h-5 w-5 text-superseller-cyan" />
          ...
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-1">
              <div className="relative">
                <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-superseller-text-tertiary" />
                <Input
                  placeholder="Search agents..."
                  value={searchTerm}
                  onChange={(e) => setSearchTerm(e.target.value)}
                  className="pl-10 bg-white/5 border-white/10 text-white placeholder-gray-500 focus:border-superseller-cyan focus:ring-superseller-cyan"
                />
              </div>
            </div>
            <select
      ...
      <Card variant="supersellerNeon" className="superseller-card-neon">
        <CardHeader>
          <CardTitle className="text-superseller-text-primary">All Agents</CardTitle>
          <CardDescription className="text-superseller-text-secondary">Manage and monitor your deployed AI services</CardDescription>
        </CardHeader>
        <CardContent>
          {loading ? (
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-white/10 hover:bg-white/5">
                  <TableHead className="text-superseller-text-secondary">Name</TableHead>
                  ...
                  <TableHead className="text-superseller-text-secondary">Schedule</TableHead>
                  <TableHead className="text-superseller-text-secondary">Created</TableHead>
              ...
              <TableBody>
                {filteredAgents.map((agent) => (
                  <TableRow key={agent.id} className="border-white/5 hover:bg-white/5">
                    ...
                    <TableCell>
                      <div className="flex items-center space-x-1 text-superseller-text-secondary">
                        <Activity className="h-4 w-4 text-superseller-text-tertiary" />
                        <span className="text-sm">{formatDate(agent.createdAt)}</span>
                      </div>
                    </TableCell>
                    <TableCell>
                      <div className="flex items-center space-x-2">
                        <Button variant="supersellerSecondary" size="sm" title="Configure">
                          <Settings className="h-4 w-4" />
                        </Button>
                        <Button variant="supersellerSecondary" size="sm" title="Run now">
