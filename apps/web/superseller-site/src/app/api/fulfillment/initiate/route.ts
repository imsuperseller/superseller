...
const N8N_FULFILLMENT_WEBHOOK = process.env.N8N_FULFILLMENT_WEBHOOK_URL || 'https://n8n.superseller.agency/webhook/fulfillment-orchestrator';
...
export async function POST(request: Request) {
    try {
        ...
        if (targetWebhook && !targetWebhook.startsWith('http')) {
            targetWebhook = `https://n8n.superseller.agency/webhook/${targetWebhook}`;
        }
        ...
        if (clientEmail) {
            try {
                await emails.fulfillmentStarted(clientEmail, clientId, productName || 'SuperSeller AI Service');
            } catch (emailError) {
