...
export async function GET() {
    ...
    try {
        ...
        const n8nUrl = 'https://n8n.superseller.agency/api/v1/healthz';
        ...
        const serverData = await sshExec(`
            ...
            echo "---LOG_ERRORS---"
            docker logs --tail 500 n8n_superseller 2>&1 | grep -iE "error|timeout|oom" | wc -l
            
            echo "---BACKUPS---"
            ls -dt /root/n8n-backups/*/ | head -n 5 | xargs -I {} basename {}
            
            echo "---VERSION---"
            docker exec n8n_superseller n8n --version
        `);
...
export async function POST(req: NextRequest) {
    ...
    if (action === 'restart' || action === 'manual-backup' || action === 'prune' || action === 'rebuild') {
        let cmd = '';
        switch (action) {
            case 'restart': cmd = 'cd /opt/n8n && docker compose restart n8n_superseller'; break;
            case 'manual-backup': cmd = 'bash /opt/n8n/superseller-n8n-upgrade.sh backup-only'; break;
            case 'prune': cmd = 'docker exec n8n_superseller n8n executions:prune --days 7 && docker system prune -f'; break;
    ...
    const stream = new ReadableStream({
        start(controller) {
            ...
            const child = spawn(sshpassPath, [
                ...
                `root@${serverIp}`,
                `bash /opt/n8n/superseller-n8n-upgrade.sh ${targetVersion} --non-interactive`
            ]);
