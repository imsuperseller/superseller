...
export default function CompeteFeed({ tenantSlug, userName }: Props) {
  ...
  if (loading) {
    return (
      <div dir="rtl" className="min-h-screen flex flex-col items-center justify-center" style={{ background: "var(--superseller-bg-primary)" }}>
        ...
        <p className="text-sm mt-5" style={{ color: "var(--superseller-text-muted)" }}>טוען...</p>
      </div>
    );
  }
    ...
    return (
      <div dir="rtl" className="min-h-screen flex items-center justify-center px-4" style={{ background: "var(--superseller-bg-primary)" }}>
        <div className="text-center">
          ...
          <p className="text-sm" style={{ color: "var(--superseller-text-muted)" }}>הפרסומות של {displayName} עדיין לא נאספו.</p>
        </div>
      </div>
    );
  }
  ...
  if (showHistory) {
    return (
      <div dir="rtl" className="min-h-screen" style={{ background: "var(--superseller-bg-primary)" }}>
        <div className="sticky top-0 z-20 px-4 py-3 flex items-center justify-between backdrop-blur-xl" style={{ background: "rgba(13,27,46,0.9)", borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
          ...
          <span className="text-[11px] font-semibold" style={{ color: "var(--superseller-text-muted)" }}>{reviewed} דירוגים</span>
        </div>
        <div className="max-w-lg mx-auto px-4 py-4 space-y-3">
          {reviewedAds.map((ad, idx) => (
            ...
            >
              <CompetitorAdCard ad={ad} onRate={handleRate} reviewerName={reviewerName} compact />
            </motion.div>
          ))}
        </div>
      </div>
    ...
    return (
      <div dir="rtl" className="min-h-screen flex items-center justify-center px-4 relative overflow-hidden" style={{ background: "var(--superseller-bg-primary)" }}>
        ...
        >
          ...
          <h2 className="text-3xl font-black mb-3" style={{ color: "var(--superseller-text-primary)" }}>סיימתם!</h2>
          <p className="text-base mb-2" style={{ color: "var(--superseller-text-secondary)" }}>
            סקרתם <span className="font-black" style={{ color: "var(--superseller-cyan)" }}>{total}</span> פרסומות
          </p>
          <div className="flex justify-center gap-6 mb-8">
            <span className="text-sm"><span className="font-black" style={{ color: "var(--superseller-teal)" }}>{likedCount}</span> <span style={{ color: "var(--superseller-text-muted)" }}>אהבתי</span></span>
            <span className="text-sm"><span className="font-black" style={{ color: "var(--superseller-orange)" }}>{dislikedCount}</span> <span style={{ color: "var(--superseller-text-muted)" }}>לא אהבתי</span></span>
          </div>
  ...
  // ─── MAIN FEED ───
  return (
    <div dir="rtl" className="min-h-screen flex flex-col relative" style={{ background: "var(--superseller-bg-primary)" }}>
        ...
        <div className="max-w-lg mx-auto flex items-center justify-between">
          <div>
            ...
            <p className="text-[10px] font-semibold" style={{ color: "var(--superseller-text-muted)" }}>
              {total} פרסומות &middot; {pendingAds.length} ממתינות
            </p>
          </div>
      ...
      <div className="flex-1 flex items-start justify-center px-4 pt-4 pb-6 relative">
        <div className="max-w-lg w-full relative">
          {currentIndex > 0 && (
            <button onClick={goPrev} className="absolute -right-14 top-48 w-10 h-10 rounded-full flex items-center justify-center cursor-pointer backdrop-blur-md hover:bg-white/[0.06] transition-all duration-300 hidden lg:flex" style={{ color: "var(--superseller-text-muted)", border: "1px solid rgba(255,255,255,0.06)" }}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M9 18l6-6-6-6"/></svg>
            </button>
          )}
          {currentIndex < pendingAds.length - 1 && (
            <button onClick={goNext} className="absolute -left-14 top-48 w-10 h-10 rounded-full flex items-center justify-center cursor-pointer backdrop-blur-md hover:bg-white/[0.06] transition-all duration-300 hidden lg:flex" style={{ color: "var(--superseller-text-muted)", border: "1px solid rgba(255,255,255,0.06)" }}>
