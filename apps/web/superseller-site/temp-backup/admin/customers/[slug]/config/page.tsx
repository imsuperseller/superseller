...
export default function CustomerConfigPage() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-superseller-bg text-superseller-text flex items-center justify-center">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-superseller-primary mx-auto mb-4"></div>
          <p className="text-superseller-text-muted">Loading configuration...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-superseller-bg text-superseller-text flex items-center justify-center">
        <div className="text-center">
          ...
          <h1 className="text-2xl font-bold text-superseller-text mb-2">Configuration Not Found</h1>
          <p className="text-superseller-text-muted mb-4">
  ...
  return (
    <div className="min-h-screen bg-superseller-bg text-superseller-text">
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <h1 className="text-3xl font-bold text-superseller-text mb-2">
              {config.company} Configuration
            </h1>
            <p className="text-superseller-text-muted">
          ...
          {[
            ...
            <button
              ...
              className={`flex items-center space-x-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                activeTab === tab.id
                  ? 'bg-superseller-primary text-white'
                  : 'bg-superseller-card text-superseller-text-muted hover:text-superseller-text hover:bg-superseller-card/80'
              }`}
            >
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {Object.entries(config.features).map(([feature, enabled]) => (
                    <div key={feature} className="flex items-center justify-between p-4 border border-superseller-border rounded-lg">
                      <div>
                        <h3 className="font-medium text-superseller-text capitalize">
                          {feature.replace(/([A-Z])/g, ' $1').trim()}
                        </h3>
                        <p className="text-sm text-superseller-text-muted">
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-3">
                  {config.tabs.map((tab, index) => (
                    <div key={tab.id} className="flex items-center space-x-3 p-3 border border-superseller-border rounded-lg">
                      <Input
                        value={tab.icon}
                        onChange={(e) => {
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-3">
                  {config.agents.map((agent, index) => (
                    <div key={agent.id} className="p-4 border border-superseller-border rounded-lg">
              ...
              <CardContent className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {config.availableIntegrations?.map((integration) => (
                    <div key={integration} className="p-3 border border-superseller-border rounded-lg">
                      <div className="flex items-center justify-between">
                        <span className="text-superseller-text">{integration}</span>
                        <Badge variant="outline">Available</Badge>
                      </div>
                    </div>
                  )) || (
                    <p className="text-superseller-text-muted col-span-2">
