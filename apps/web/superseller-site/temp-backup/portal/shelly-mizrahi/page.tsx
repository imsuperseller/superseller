...
import { supersellerStyles, componentVariants } from '@/lib/superseller-styles';
...
export default function ShellyMakeComPortal() {
  ...
  return (
    <div className={`min-h-screen ${supersellerStyles.bgPrimary} p-6`}>
      <div className="max-w-6xl mx-auto">
        <div className={`mb-8 p-6 rounded-lg ${supersellerStyles.bgCard} border border-superseller-bg-secondary`}>
          <h1 className={`text-3xl font-bold ${supersellerStyles.textPrimary} mb-2`}>
            Shelly Mizrahi - Make.com Family Research
          </h1>
          <p className={`text-lg ${supersellerStyles.textSecondary}`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Card className={`${componentVariants.card.default}`}>
            <CardHeader>
              <CardTitle className={`${supersellerStyles.textPrimary}`}>
            ...
            <CardContent>
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  <Label htmlFor="clientId" className={`${supersellerStyles.textPrimary}`}>
                    Client ID
                  </Label>
                  <Input
                    id="clientId"
                ...
                <div>
                  <Label htmlFor="familyMemberIds" className={`${supersellerStyles.textPrimary}`}>
                    Family Member IDs
                  </Label>
                  <Textarea
                    id="familyMemberIds"
                ...
                <div>
                  <Label htmlFor="researchDepth" className={`${supersellerStyles.textPrimary}`}>
                    Research Depth
                  </Label>
                  <select
                    id="researchDepth"
                    ...
                    className={`w-full p-2 rounded border ${supersellerStyles.bgSecondary} ${supersellerStyles.textPrimary}`}
                  >
                ...
                <Button
                  ...
                  className={`${componentVariants.button.supersellerPrimary} w-full`}
                >
            ...
            <CardContent>
              <div className="space-y-4">
                {profiles.map((profile) => (
                  <div key={profile.id} className={`p-4 rounded border ${supersellerStyles.bgSecondary}`}>
                    <div className="flex items-center justify-between mb-2">
                      <h3 className={`font-semibold ${supersellerStyles.textPrimary}`}>
                        {profile.familyName}
                      </h3>
                      {getStatusBadge(profile.status)}
                    </div>
                    <p className={`text-sm ${supersellerStyles.textSecondary} mb-2`}>
                    ...
                    {profile.documentUrl && (
                      <Button
                        ...
                        className={`${componentVariants.button.supersellerSecondary} text-sm`}
                      >
                ...
                {profiles.length === 0 && (
                  <p className={`text-center ${supersellerStyles.textMuted}`}>
