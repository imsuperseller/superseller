...
async function main() {
  console.log('🌱 Seeding SuperSeller AI database...\n');

  // ── 1. Users ──────────────────────────────────────────────
  const adminUser = await prisma.user.upsert({
    where: { id: 'admin.superseller.agency' },
    update: {},
    create: {
      id: 'admin.superseller.agency',
  ...
  const legacyClient = await prisma.client.upsert({
    ...
    create: {
      id: 'coolair-hvac',
      name: 'CoolAir HVAC',
      logoUrl: 'https://superseller.agency/logos/coolair.png',
  ...
  const purchase = await prisma.purchase.create({
    data: {
      ...
      downloadUrl: 'https://superseller.agency/api/marketplace/download/dGVzdC10b2tlbi0wMDE',
      tier: 'full-service',
    },
  });
  ...
  await prisma.testimonial.create({
    data: {
      ...
      quote: 'SuperSeller AI automated our entire lead response system. We went from 30-minute response times to under 5 minutes.',
