...
export default function UsersPage() {
  ...
  const getRoleBadge = (role: string) => {
    switch (role) {
      case 'owner':
        return <Badge variant="renstoPrimary">Owner</Badge>;
      case 'admin':
        return <Badge variant="renstoSecondary">Admin</Badge>;
      case 'user':
        return <Badge variant="renstoNeon">User</Badge>;
      case 'viewer':
        return <Badge variant="renstoWarning">Viewer</Badge>;
      default:
  ...
  const getMfaBadge = (mfa: boolean) => {
    return mfa ? (
      <Badge variant="renstoSuccess">Enabled</Badge>
    ) : (
      <Badge variant="renstoError">Disabled</Badge>
    );
  };
            ...
            <TableBody>
              {filteredUsers.map((user) => (
                <TableRow key={user.id}>
                  ...
                  <TableCell>
                    <div className="flex items-center space-x-2">
                      <Button variant="renstoSecondary" size="sm" title="Send magic link">
                        <Mail className="h-4 w-4" />
                      </Button>
                      <Button
                        variant={user.mfa ? "renstoError" : "renstoSuccess"}
                        size="sm"
                        title={user.mfa ? "Disable MFA" : "Enable MFA"}
                      >
                        <Shield className="h-4 w-4" />
                      </Button>
                      <Button variant="renstoSecondary" size="sm" title="Revoke sessions">
