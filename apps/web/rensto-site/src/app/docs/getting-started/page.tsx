  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: 'var(--rensto-bg-primary)' }}>
      ...
      <main className="flex-grow">
        ...
        {/* Quick Start Steps */}
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <div className="rounded-lg shadow-sm border p-8 mb-8" style={{ background: 'var(--rensto-bg-card)', borderColor: 'rgba(255, 255, 255, 0.1)' }}>
            <h2 className="text-2xl font-bold mb-6" style={{ color: 'var(--rensto-text-primary)' }}>Quick Start Guide</h2>

            <div className="space-y-8">
              {steps.map((step, index) => (
                <div key={step.title} className="flex items-start space-x-4">
                  <div className="flex-shrink-0">
                    <div className="w-10 h-10 rounded-full flex items-center justify-center" style={{ background: 'rgba(0, 242, 254, 0.1)', color: 'var(--rensto-cyan)' }}>
                  ...
                  <div className="flex-1">
                    <div className="flex items-center mb-2">
                      <step.icon className="h-5 w-5 mr-2" style={{ color: 'var(--rensto-blue)' }} />
                      <h3 className="text-lg font-semibold" style={{ color: 'var(--rensto-text-primary)' }}>{step.title}</h3>
                    </div>
                    <p className="mb-3" style={{ color: 'var(--rensto-text-secondary)' }}>{step.description}</p>

                    <ul className="space-y-2">
                      {step.details.map((detail, detailIndex) => (
                        <li key={detailIndex} className="flex items-start">
                          <CheckCircle className="h-4 w-4 text-green-500 mr-2 mt-0.5 flex-shrink-0" />
                          <span className="text-sm" style={{ color: 'var(--rensto-text-secondary)' }}>{detail}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
          ...
          {/* Key Features */}
          <div className="rounded-lg shadow-sm border p-8 mb-8" style={{ background: 'var(--rensto-bg-card)', borderColor: 'rgba(255, 255, 255, 0.1)' }}>
            <h2 className="text-2xl font-bold mb-6" style={{ color: 'var(--rensto-text-primary)' }}>Key Features</h2>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {features.map((feature) => (
                <div key={feature.title} className="border rounded-lg p-6" style={{ borderColor: 'rgba(255, 255, 255, 0.1)' }}>
                  <h3 className="text-lg font-semibold mb-2" style={{ color: 'var(--rensto-text-primary)' }}>{feature.title}</h3>
                  <p className="mb-4" style={{ color: 'var(--rensto-text-secondary)' }}>{feature.description}</p>

                  <ul className="space-y-2">
                    {feature.benefits.map((benefit, index) => (
                      <li key={index} className="flex items-start">
                        <CheckCircle className="h-4 w-4 text-green-500 mr-2 mt-0.5 flex-shrink-0" />
                        <span className="text-sm" style={{ color: 'var(--rensto-text-secondary)' }}>{benefit}</span>
                      </li>
                    ))}
                  </ul>
                </div>
          ...
          <div className="rounded-lg p-8" style={{ background: 'linear-gradient(to right, rgba(0, 242, 254, 0.05), rgba(79, 172, 254, 0.05))', border: '1px solid rgba(255, 255, 255, 0.1)' }}>
            <h2 className="text-2xl font-bold mb-4" style={{ color: 'var(--rensto-text-primary)' }}>What&apos;s Next?</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <Link
                href="/docs/agents"
                className="block p-4 rounded-lg border border-white/10 hover:border-[var(--rensto-blue)]/50 hover:bg-white/5 transition-colors"
              >
                <h3 className="font-semibold mb-2" style={{ color: 'var(--rensto-text-primary)' }}>Agent Management</h3>
                <p className="text-sm" style={{ color: 'var(--rensto-text-secondary)' }}>Learn how to create and manage automation agents</p>
              </Link>
              ...
              <Link
                ...
                className="block p-4 rounded-lg border border-white/10 hover:border-[var(--rensto-blue)]/50 hover:bg-white/5 transition-colors"
              >
                <h3 className="font-semibold mb-2" style={{ color: 'var(--rensto-text-primary)' }}>Analytics Guide</h3>
                <p className="text-sm" style={{ color: 'var(--rensto-text-secondary)' }}>Understand your data and get actionable insights</p>
              </Link>
              ...
              <Link
                ...
                className="block p-4 rounded-lg border border-white/10 hover:border-[var(--rensto-blue)]/50 hover:bg-white/5 transition-colors"
              >
                <h3 className="font-semibold mb-2" style={{ color: 'var(--rensto-text-primary)' }}>API Reference</h3>
                <p className="text-sm" style={{ color: 'var(--rensto-text-secondary)' }}>Integrate Rensto with your existing systems</p>
              </Link>
            </div>
          </div>

          {/* Support */}
          <div className="mt-8 rounded-lg shadow-sm border p-8" style={{ background: 'var(--rensto-bg-card)', borderColor: 'rgba(255, 255, 255, 0.1)' }}>
            <div className="text-center">
