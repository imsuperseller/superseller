...
export function CrewMemberDetail({ member }: CrewMemberDetailProps) {
  ...
  return (
    <div className="space-y-16">
      ...
      <Link
        href="/crew"
        className="inline-flex items-center gap-2 text-sm text-[var(--rensto-text-muted)] hover:text-white transition-colors"
      >
      ...
      >
        ...
        <p className="text-xl text-[var(--rensto-text-secondary)] max-w-2xl leading-relaxed">
          {member.description}
        </p>
      ...
      {/* Demo video — real output example */}
      {member.demoVideo && (
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          ...
          {member.demoCaption && (
            <p className="text-sm text-[var(--rensto-text-muted)] mt-3 text-center italic">
              {member.demoCaption}
            </p>
          )}
        </motion.div>
          ...
          {member.features.map((feature, i) => (
            ...
            >
              <Check
                ...
                style={{ color: member.accentColor }}
              />
              <span className="text-sm text-[var(--rensto-text-secondary)]">
                {feature}
              </span>
            </div>
          ))}
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {relevantNiches.map((niche) => (
              ...
              >
                <h3 className="text-lg font-bold text-white mb-2 group-hover:text-[var(--rensto-accent-cyan)] transition-colors">
                  {niche.name}
                </h3>
                <p className="text-sm text-[var(--rensto-text-muted)] mb-3">
                  <strong className="text-[var(--rensto-text-secondary)]">
                    Pain:
                  </strong>{' '}
      ...
      >
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
          <Link href="/pricing">
            <Button
              size="xl"
              className="rensto-btn-3d-accent font-black uppercase tracking-[0.15em] rounded-2xl cursor-pointer text-white"
              style={{
