...
export function Footer() {
  ...
  const navigation = {
    ...
    social: [
      ...
      {
        name: 'WhatsApp',
        href: `https://wa.me/${CONTACT_WHATSAPP}?text=Hi%20Rensto%2C%20I'm%20interested%20in%20your%20AI%20crew.`,
        icon: MessageCircle,
      },
    ],
  };
    ...
    >
      <div className="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-5 gap-8">
            ...
            <div className="flex space-x-4">
              {navigation.social.map((item) => (
                <Link
                  key={item.name}
                  href={item.href}
                  className="text-[var(--rensto-text-muted)] hover:text-white transition-colors"
                  target="_blank"
          ...
          {/* Navigation */}
          <div>
            <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-[var(--rensto-text-muted)] mb-4">
              Navigation
            </h3>
            <ul className="space-y-3">
              {navigation.main.map((item) => (
                <li key={item.name}>
                  <Link
                    href={item.href}
                    className="text-sm text-[var(--rensto-text-secondary)] hover:text-white transition-colors"
                  >
                    {item.name}
                  </Link>
                </li>
            ...
            <div className="flex space-x-6">
              {navigation.legal.map((item) => (
                <Link
                  key={item.name}
                  href={item.href}
                  className="text-[var(--rensto-text-muted)] hover:text-white transition-colors text-sm"
                >
                  {item.name}
                </Link>
              ))}
            </div>
            <p className="text-[var(--rensto-text-muted)] text-sm">
              &copy; {currentYear} Rensto. All rights reserved.
            </p>
          </div>
        </div>
      </div>
