...
export function CustomizationModal({
    ...
    React.useEffect(() => {
        if (isOpen) {
            const savedState = localStorage.getItem(`rensto_discovery_${workflowId}`);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (data.success) {
                ...
                setTimeout(() => {
                    setFlowState('result');
                    localStorage.setItem(`rensto_discovery_${workflowId}`, 'result');
    ...
    const handleReset = () => {
        localStorage.removeItem(`rensto_discovery_${workflowId}`);
        setFlowState('form');
        setFormData({});
        setProgress(0);
    };
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <Button
                                    ...
                                    className="h-16 border-white/10 text-white hover:bg-white/5 rounded-2xl font-black uppercase tracking-tight text-xs"
                                    onClick={() => {
                                        const bookedCount = parseInt(localStorage.getItem('rensto_strategy_booked') || '0');
                                        const url = bookedCount === 0
                                            ? '/contact?type=strategy'
                                            : '/contact?type=strategy-premium';

                                        window.open(url, '_blank');
                                        localStorage.setItem('rensto_strategy_booked', (bookedCount + 1).toString());
                                    }}
                                >
                                    {typeof window !== 'undefined' && parseInt(localStorage.getItem('rensto_strategy_booked') || '0') > 0
                                        ? t.schedule
                                        : t.strategySession}
                                </Button>
                            </div>
