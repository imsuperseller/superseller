...
export function CreditSlider() {
  ...
  return (
    ...
    >
      ...
      <p className="text-sm text-[var(--rensto-text-muted)] text-center mb-8">
          ...
          return (
            <div key={slider.crewId} className="space-y-2">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="text-[var(--rensto-text-muted)] tabular-nums">
                  {val} {slider.unit}{' '}
              ...
              <input
                ...
                style={{
                  background: `linear-gradient(to right, ${member?.accentColor ?? 'var(--rensto-accent-cyan)'} 0%, ${member?.accentColor ?? 'var(--rensto-accent-cyan)'} ${((val - slider.min) / (slider.max - slider.min)) * 100}%, rgba(255,255,255,0.1) ${((val - slider.min) / (slider.max - slider.min)) * 100}%, rgba(255,255,255,0.1) 100%)`,
                }}
              />
            </div>
          );
      ...
      <div className="mt-8 pt-6 border-t border-white/10">
        <div className="flex items-center justify-between mb-4">
          <span className="text-[var(--rensto-text-muted)] text-sm font-bold uppercase tracking-[0.15em]">
        ...
        >
          <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[var(--rensto-text-muted)] mb-1">
            Recommended Plan
          </p>
          <p className="text-lg font-black text-white">
            {recommendedPlan.name}{' '}
            <span style={{ color: 'var(--rensto-accent-cyan)' }}>
              ${recommendedPlan.price}/mo
            </span>
          </p>
          <p className="text-xs text-[var(--rensto-text-muted)] mt-1">
