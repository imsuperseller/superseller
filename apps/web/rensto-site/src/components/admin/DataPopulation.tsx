...
export default function DataPopulation({ className = '' }: DataPopulationProps) {
  ...
  const getStatusIcon = () => {
    switch (status) {
      case 'loading':
        return <Loader2 className="h-4 w-4 rensto-animate-glow" />;
      case 'success':
        return <CheckCircle className="h-4 w-4 text-green-600" />;
      case 'error':
        return <AlertTriangle className="h-4 w-4 style={{ color: 'var(--rensto-red)' }}" />;
      default:
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex items-center gap-4">
            ...
            >
              {loading ? (
                <Loader2 className="h-4 w-4 rensto-animate-glow" />
              ) : (
