...
```bash
# ❌ This hangs:
ssh root@173.254.201.134 "command"

# ✅ This works:
/bin/sh -c "ssh root@173.254.201.134 'command'"
```
...
/bin/sh -c "ssh root@173.254.201.134 'docker ps'"
/bin/sh -c "scp file.csv root@173.254.201.134:/tmp/"
/bin/sh -c "ping -c 3 173.254.201.134"
```

### Using Safe Exec Wrapper
```bash
# Use the safe-exec wrapper:
bash scripts/safe-exec.sh "ssh root@173.254.201.134 'docker ps'"
bash scripts/safe-exec.sh "scp file.csv root@173.254.201.134:/tmp/"
```
...
✅ **SSH commands**: `ssh root@173.254.201.134 'echo OK'` - Works  
✅ **SCP uploads**: `scp file.csv root@173.254.201.134:/tmp/` - Works  
✅ **Docker commands**: `docker exec n8n_rensto n8n --version` - Works  
✅ **Ping tests**: `ping -c 2 173.254.201.134` - Works  
