...
> **Current Stack (Feb 2026)**: **Kie.ai Kling 3.0** (video), **Resend magic-link** (auth), **Ollama + pgvector** (RAG), **Gemini Flash** (LLM). See `apps/worker/src/services/kie.ts` and `apps/worker/src/services/rag.ts`. The architecture and pipeline flow below remain accurate.
---
## TABLE OF CONTENTS
