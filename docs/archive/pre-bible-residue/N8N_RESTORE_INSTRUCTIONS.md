...
1. **Go to**: http://172.245.56.50:5678/setup
2. **Fill in**:
...
2. **Import workflows**:
   ```bash
   curl -X POST http://172.245.56.50:5678/api/v1/workflows/import \
...
```bash
curl -X POST http://172.245.56.50:5678/api/v1/credentials \
  -H "X-N8N-API-KEY: your-api-key" \
  -H "Content-Type: application/json" \
