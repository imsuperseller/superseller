...
2. **Copy CSVs to n8n container**:
   ```bash
   scp "n8n Credentials-Grid view.csv" root@172.245.56.50:/tmp/
   ssh root@172.245.56.50 'docker cp /tmp/n8n\ Credentials-Grid\ view.csv n8n_rensto:/tmp/n8n-data/n8n-credentials.csv'
...
3. **Execute workflow 3 times** (once per CSV):
   ...
   # Execute via webhook
   curl -X POST "http://172.245.56.50:5678/webhook/[WORKFLOW_ID]/webhook/execute-infrastructure-import" \
