...
```javascript
voice_url: "={{ ($json.payload?._data?.message?.audioMessage?.ptt === true || ($json.payload?.hasMedia === true && $json.payload?.media?.mimetype?.includes('audio'))) ? $json.payload?.media?.url?.replace('localhost:3000', '173.254.201.134:3000') : '' }}"
```
