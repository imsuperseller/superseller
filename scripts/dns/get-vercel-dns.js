...
const PROJECT_NAME = 'rensto-site';
...
async function main() {
  ...
  try {
    ...
    await addDomain('rensto.com', projectId);
    await addDomain('www.rensto.com', projectId);
    
    // Get domain configurations
    log('\n📋 Step 3: Fetching DNS configuration...', 'blue');
    const rootConfig = await getDomainConfig('rensto.com');
    const wwwConfig = await getDomainConfig('www.rensto.com');
    ...
    if (wwwConfig) {
      log('\n🌐 WWW Subdomain (www.rensto.com):', 'blue');
    ...
    log('\nFor root domain (rensto.com), you have two options:', 'blue');
    ...
    log('\nUpdate scripts/dns/migrate-rensto-to-vercel.js:', 'blue');
    log('', 'reset');
    ...
    if (domains.length > 0) {
      ...
      log('  Run: vercel domains add rensto.com --project=rensto-site', 'cyan');
    }
