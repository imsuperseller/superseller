...
# - ❌ DON'T run database cleanup during update (lost execution history on Nov 12, 2025)
...
# Step 6: Backup data volume (CRITICAL - includes database, user accounts, settings)
echo ""
echo "💾 Backing up data volume (includes database, user accounts, settings)..."
if docker volume inspect "$VOLUME_NAME" >/dev/null 2>&1; then
    ...
    if docker run --rm -v "$VOLUME_NAME:/data" -v "$BACKUP_DIR:/backup" alpine \
        ...
        echo "   This includes: database, user accounts, settings, execution history"
    else
...
cat > "$BACKUP_DIR/backup-manifest.json" << EOF
{
    ...
    "status": "completed",
    "notes": "Data volume backup includes database, user accounts, settings, and execution history. Login credentials are preserved in database backup."
}
EOF
...
echo "Target Version: $TARGET_VERSION"
echo ""
echo "⚠️  IMPORTANT: Login credentials are preserved in the data volume backup."
echo "   No need to recreate admin account after update."
...
echo ""
echo "📋 Next Steps:"
echo "1. Verify you can login (credentials preserved in database backup)"
echo "2. Verify workflows are working: http://173.254.201.134:5678"
echo "3. Check community nodes are still installed"
echo "4. Test a few workflows to ensure everything works"
echo "5. If issues occur, restore from backup: $BACKUP_DIR"
echo ""
echo "🛡️  Your data is safely backed up and n8n has been updated!"
echo "   Login credentials are preserved - no need to recreate admin account."
