 ...
 * Migrates superseller.agency from Webflow to Vercel
 * 
 * Usage:
...
async function migrateToVercel(dryRun = false) {
  ...
  try {
    ...
    if (dryRun) {
      ...
      log('  node scripts/dns/migrate-superseller-to-vercel.js --execute', 'bright');
    } else {
      ...
      if (allSuccess) {
        ...
        log('  4. Test superseller.agency and www.superseller.agency', 'cyan');
...
async function rollback() {
  ...
  try {
    ...
    log('\n📊 Next Steps:', 'blue');
    log('  1. Wait 5-30 minutes for DNS propagation', 'cyan');
    log('  2. Verify superseller.agency loads from Webflow', 'cyan');
    log('='.repeat(60) + '\n', 'bright');
...
if (command === '--dry-run' || command === '--dryrun') {
  ...
  log('DNS Migration Script for superseller.agency', 'bright');
  log('');
  log('Usage:', 'cyan');
