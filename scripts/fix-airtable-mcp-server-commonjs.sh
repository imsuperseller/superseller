...
print_status "Fixing Airtable MCP Server with CommonJS..."

# Step 1: Stop the current service
print_status "Stopping current service..."
run_ssh "systemctl stop $SERVICE_NAME"

# Step 2: Create a CommonJS webhook wrapper
print_status "Creating CommonJS webhook wrapper..."
run_ssh "cat > /tmp/commonjs-webhook-wrapper.js << 'EOF'
...
ExecStart=/usr/bin/node /tmp/commonjs-webhook-wrapper.js
Restart=always
RestartSec=10
