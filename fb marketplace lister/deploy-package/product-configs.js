...
const MISSPARTY_PRICE = 75; // Fixed — NO jitter
...
function getMissPartyReferenceImage() {
    const existing = MISSPARTY_BASE_IMAGES.filter(p => fs.existsSync(p));
    return existing.length > 0 ? pick(existing) : null;
}
...
async function generateUniqueUadConfig(pool) {
    ...
    for (let attempt = 0; attempt < 50; attempt++) {
        ...
        const prompts = {
            prompt1: `${color} ${collection} garage door, ${design} design, professional installation on Texas suburban home, bright daylight, curb appeal, realistic photo`,
            prompt2: `Close up detail of ${color} ${collection} garage door, ${design} pattern, high resolution, suburban home exterior`,
            prompt3: `Wide shot: beautiful ${color} ${collection} garage door on Texas home, ${design} design, landscaped driveway, professional photography`,
        };
    ...
    return {
        ...
        prompts: {
            prompt1: `${color} ${collection} garage door, ${design} design, professional installation on Texas suburban home, bright daylight`,
            prompt2: `Close up of ${color} ${collection} garage door, ${design} pattern, high resolution suburban photo`,
            prompt3: `Wide shot: ${color} ${collection} garage door on Texas home, ${design} design, curb appeal`,
        },
        productName: `${size} ${collection} Garage Door`,
        configData: { collection, size, design, color, construction },
    };
...
function generateMissPartyConfig(scenarioIndex) {
    ...
    const prompts = {
        prompt1: `White inflatable bounce house, ${scenario.desc}, realistic photo, happy children, professional photography, bright natural lighting`,
        prompt2: `White bouncy castle ${locationDesc}, ${kidsDesc}, ${ballsDesc}, lifestyle photography`,
        prompt3: `White inflatable bouncer rental setup, ${scenario.setting === 'outdoors' ? 'outdoor party setting' : 'indoor celebration'}, ${scenario.kids === 'many' ? 'birthday party atmosphere' : 'small family gathering'}, professional product photo`,
    };
