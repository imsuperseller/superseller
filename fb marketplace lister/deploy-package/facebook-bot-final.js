...
class ManualFacebookBot {
    ...
    async launchBrowser() {
        ...
        // Pre-create fonts directory to prevent ENOENT crash (matches working V13)
        ...
        try {
            // GoLogin config — matches the WORKING V13 master-bot.js exactly:
            // - NO tmpdir (default /tmp)
            // - NO executablePath (SDK auto-detects)
            // - NO writeCookiesFromServer (working version didn't have it)
            // - --display=:100 for X11 on headless VPS
            // - uploadCookiesToServer: false (never corrupt S3 profile)
            // - writeCookesToServer: false (don't read encrypted Chromium cookies)
            ...
            console.log(`[${this.config.name}] Starting GoLogin profile ${this.config.profileId}...`);

            const { status, wsUrl } = await GL.start();

            console.log(`[${this.config.name}] GoLogin status: ${status}`);
            ...
            console.log(`[${this.config.name}] Connected to GoLogin browser`);
            ...
            console.error(`[${this.config.name}] GoLogin SDK failed: ${error.message}`);
            throw new Error(`GoLogin SDK failed: ${error.message}`);
        }
    }
    ...
    // Backup: save cookies to local file (survives GoLogin API outages)
    async saveCookiesToFile(page) {
        try {
            ...
            fs.writeFileSync(this.cookieFilePath, JSON.stringify(formatted, null, 2));
            console.log(`[${this.config.name}] Saved ${formatted.length} cookies to ${path.basename(this.cookieFilePath)}`);
            return true;
        } catch (e) {
    ...
    async checkFacebookLogin(page) {
        try {
            ...
            const url = page.url();
            const needsLogin = url.includes("login") || await page.$("#email").catch(() => null);
    ...
    async attemptDirectFacebookLogin(page) {
        ...
        try {
            console.log(`[${this.config.name}] Attempting direct Facebook login...`);

            await page.goto("https://www.facebook.com/login", { waitUntil: "networkidle2", timeout: 30000 });

            // Use #email/#pass selectors with page.type() — matches working V13
            // The working version used these selectors and they posted 100+ listings
            const emailField = await page.waitForSelector("#email", { timeout: 10000 }).catch(() => null);
            if (!emailField) {
                console.log(`[${this.config.name}] #email not found, trying input[name="email"]`);
                await page.waitForSelector('input[name="email"]', { timeout: 10000 });
            }

            await page.type(emailField ? "#email" : 'input[name="email"]', this.config.fbEmail);
            await page.type(emailField ? "#pass" : 'input[name="pass"]', this.config.fbPass);
            console.log(`[${this.config.name}] Credentials typed`);
            await delay(randomRange(800, 1500));

            // Submit via button click — matches working V13
            await page.click('button[name="login"]').catch(async () => {
                // Fallback: Enter key
                console.log(`[${this.config.name}] button[name="login"] not found, using Enter key`);
                await page.focus(emailField ? "#pass" : 'input[name="pass"]');
                await page.keyboard.press('Enter');
            });
            console.log(`[${this.config.name}] Login submitted`);
            ...
            if (currentUrl.includes("auth_platform") || currentUrl.includes("checkpoint") || currentUrl.includes("security")) {
                console.log(`[${this.config.name}] 2FA detected at: ${currentUrl}`);
                await sendWhatsApp(`Approve the Facebook login on your phone NOW! Waiting 3 min...`, this.config.name);

                for (let i = 0; i < 12; i++) {
                    await delay(15000);
                    const nowUrl = page.url();
                    console.log(`[${this.config.name}] 2FA poll ${i+1}/12 - ${nowUrl.substring(0, 80)}`);
    ...
    async postToMarketplace(page, job) {
        try {
            // Navigate directly to marketplace create — matches working V13 flow exactly
            console.log(`[${this.config.name}] Navigating to Marketplace create form...`);
            ...
            // Verify form loaded
            const formReady = await page.$('input[type="file"]');
            if (!formReady) throw new Error("Marketplace form failed to load");
