...
(async () => {
    ...
    if (loggedIn) {
        ...
        if (cu && xs) {
            ...
            fs.writeFileSync("./cookies.json", JSON.stringify(formatted, null, 2));
            console.log("   Saved " + formatted.length + " FRESH cookies to cookies.json");
            console.log("   c_user: " + cu.value);

            // Also save to GoLogin API
            try {
            ...
            // Test marketplace access with fresh session
            console.log("\nStep 6: Testing marketplace with fresh session...");
            ...
            if (hasForm) {
                ...
                console.log("\n=== Session restored but marketplace form not loaded ===");
            }
        } else {
            console.log("   WARNING: Logged in but missing c_user/xs cookies!");
        }
    } else {
