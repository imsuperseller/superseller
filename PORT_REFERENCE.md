...
|---------|------------------|---------------------|-----------------|
| **rensto-site** (Next.js) | 3002 | — | Vercel (rensto.com) |
| **Worker** (TourReel API) | 3001 | 3002 | 172.245.56.50:3002 |
| **video-merge** (RackNerd) | — | — | 3001 |

**Local conflict**: Worker and rensto-site both default to 3002. When running both, worker MUST use `PORT=3001`.
...
|-----|---------|--------------|----------|
| `VIDEO_WORKER_URL` | Worker URL for rensto-site proxy | `http://localhost:3001` | `http://172.245.56.50:3002` |
...
**Tool rule**: run-smoke, e2e-from-zillow hit **worker** directly. Worker has `/api/dev/`, `/api/jobs/`. rensto-site has `/api/video/*` (proxy).
...
# Site
cd apps/web/rensto-site && VIDEO_WORKER_URL=http://localhost:3001 pnpm dev
# Smoke
...
|---------|------|
| n8n (RackNerd) | 5678 |
| rensto-site dev | 3002 (package.json) |
