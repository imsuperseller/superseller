  ...
  const initializeOnboarding = async () => {
    ...
    try {
      ...
      addMessage({
        id: 'welcome',
        type: 'agent',
        content: `Welcome to Rensto! I'm your AI onboarding assistant. Let me analyze your profile and help you get set up with the perfect automation solution.`,
        timestamp: new Date()
      });
    ...
    return (
      <div ref={containerRef} className="flex items-center justify-center min-h-96">
        <div className="text-center space-y-4">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-rensto-cyan mx-auto"></div>
          <p className="text-rensto-text-secondary">Initializing your onboarding...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div ref={containerRef} className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-rensto-text-primary">AI Onboarding Assistant</h2>
          <p className="text-rensto-text-secondary">Let's get your automation setup perfectly configured</p>
        </div>
        <div className="flex items-center space-x-4">
          <Badge variant="renstoInfo" className="flex items-center space-x-1">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card variant="renstoNeon" className="backdrop-blur-sm bg-rensto-bg-card/50">
          <CardContent className="p-4">
            <div className="flex items-center space-x-2">
              <Target className="w-5 h-5 text-rensto-cyan" />
              <div>
                <p className="text-sm text-rensto-text-secondary">Completion</p>
        ...
        <Card variant="renstoNeon" className="backdrop-blur-sm bg-rensto-bg-card/50">
          <CardContent className="p-4">
            <div className="flex items-center space-x-2">
              <AlertCircle className="w-5 h-5 text-rensto-orange" />
              <div>
                <p className="text-sm text-rensto-text-secondary">Missing Info</p>
                <p className="text-2xl font-bold text-rensto-text-primary">
        ...
        <Card variant="renstoNeon" className="backdrop-blur-sm bg-rensto-bg-card/50">
          <CardContent className="p-4">
            <div className="flex items-center space-x-2">
              <Shield className="w-5 h-5 text-rensto-green" />
              <div>
                <p className="text-sm text-rensto-text-secondary">Risk Level</p>
                <p className="text-2xl font-bold text-rensto-text-primary capitalize">
        ...
        <Card variant="renstoNeon" className="backdrop-blur-sm bg-rensto-bg-card/50">
          <CardContent className="p-4">
            <div className="flex items-center space-x-2">
              <TrendingUp className="w-5 h-5 text-rensto-red" />
              <div>
                <p className="text-sm text-rensto-text-secondary">Current Step</p>
                <p className="text-2xl font-bold text-rensto-text-primary">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div ref={chatRef} className="lg:col-span-2">
          <Card variant="renstoNeon" className="backdrop-blur-sm bg-rensto-bg-card/50 h-96">
            <CardHeader className="pb-3">
              <CardTitle className="flex items-center space-x-2">
                <MessageSquare className="w-5 h-5 text-rensto-cyan" />
              ...
              <div className="flex-1 overflow-y-auto space-y-4 mb-4">
                  ...
                  >
                    <div
                      className={`max-w-[80%] p-3 rounded-lg ${
                        message.type === 'user'
                          ? 'bg-rensto-cyan text-rensto-bg-primary'
                          : message.type === 'system'
                          ? 'bg-red-500/10 border border-red-500/20 text-red-400'
                          : 'bg-rensto-bg-card/50 border border-rensto-text-muted/20 text-rensto-text-primary'
                      }`}
                    >
                ...
                {isProcessing && (
                  <div className="flex justify-start">
                    <div className="bg-rensto-bg-card/50 border border-rensto-text-muted/20 text-rensto-text-primary p-3 rounded-lg">
                      <div className="flex items-center space-x-2">
                        <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-rensto-cyan"></div>
            ...
            <Card variant="renstoNeon" className="backdrop-blur-sm bg-rensto-bg-card/50">
              ...
              <CardContent className="space-y-2">
                {onboardingState.missingInfo.map((item, index) => (
                  <div key={index} className="flex items-center justify-between p-2 rounded border border-rensto-text-muted/20">
                    <div className="flex items-center space-x-2">
                      {getCategoryIcon(item.category)}
                      <span className="text-sm text-rensto-text-primary">{item.field}</span>
                    </div>
            ...
            <CardContent className="space-y-2">
              {onboardingState.analysis.nextSteps.map((step, index) => (
                <div key={index} className="flex items-center space-x-2 text-sm">
                  <div className="w-2 h-2 rounded-full bg-rensto-cyan"></div>
                  <span className="text-rensto-text-secondary">{step}</span>
                </div>
              ))}
            </CardContent>
          </Card>
            ...
            <Card variant="renstoNeon" className="backdrop-blur-sm bg-rensto-bg-card/50">
              ...
              <CardContent className="space-y-2">
                {Object.entries(validationResults).map(([service, result]: [string, any]) => (
                  <div key={service} className="flex items-center justify-between p-2 rounded border border-rensto-text-muted/20">
                    <span className="text-sm text-rensto-text-primary capitalize">{service}</span>
                    {result.valid ? (
