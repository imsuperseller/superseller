...
export function TaskProgress({ tasks }: TaskProgressProps) {
  ...
  return (
    <div className="bg-rensto-card rounded-lg border border-rensto-border p-6 space-y-6">
      {/* Overall Progress */}
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-semibold text-rensto-text">Overall Progress</h3>
          <span className="text-2xl font-bold text-rensto-primary">{Math.round(overallProgress)}%</span>
        </div>
        
        <div className="space-y-2">
          <div className="flex items-center justify-between text-sm">
            <span className="text-rensto-text-muted">Progress</span>
            <span className="text-rensto-text">{completedTasks} of {totalTasks} tasks</span>
          </div>
          <div className="w-full bg-rensto-border rounded-full h-3">
            <div 
              className={`h-3 rounded-full transition-all duration-500 ${getProgressColor(overallProgress)}`}
              style={{ width: `${overallProgress}%` }}
            />
          </div>
          <p className="text-sm text-rensto-text-muted text-center">
      ...
      {(urgentTasks.length > 0 || highPriorityTasks.length > 0) && (
        <div className="space-y-3">
          <h4 className="font-medium text-rensto-text">⚠️ Priority Alerts</h4>
      ...
      {/* Time Estimates */}
      <div className="space-y-3">
        <h4 className="font-medium text-rensto-text">⏱️ Time Estimates</h4>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="p-3 bg-rensto-dark rounded-lg">
            <div className="text-sm text-rensto-text-muted">Remaining Tasks</div>
            <div className="text-lg font-semibold text-rensto-text">
              {pendingTasks + inProgressTasks} tasks
            </div>
          </div>
          <div className="p-3 bg-rensto-dark rounded-lg">
            <div className="text-sm text-rensto-text-muted">Estimated Time</div>
            <div className="text-lg font-semibold text-rensto-text">
      ...
      {/* Next Steps */}
      {pendingTasks > 0 && (
        <div className="p-4 bg-rensto-primary/10 border border-rensto-primary/20 rounded-lg">
          <h4 className="font-medium text-rensto-primary mb-2">🎯 Recommended Next Steps</h4>
          <div className="space-y-2">
            {getNextSteps(tasks).map((step, index) => (
              <div key={index} className="flex items-center space-x-2 text-sm">
                <span className="text-rensto-primary">{index + 1}.</span>
                <span className="text-rensto-text">{step}</span>
              </div>
            ))}
          </div>
        </div>
