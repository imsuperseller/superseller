...
export default function BenGinatiPortal() {
  ...
  const handleRunAgent = async (agentKey: string) => {
    try {
      ...
      const result = await runBenAgent(agentKey, {
        topic: 'Tax4Us content generation',
        language: 'hebrew',
        tone: 'professional'
      });
  ...
  const handleActivateAgent = async (agentKey: string) => {
    try {
      const result = await activateBenAgent(agentKey);
      if (result.success) {
        alert(`✅ Agent activated: ${result.message}`);
      } else {
        alert(`❌ Error: ${result.error}`);
      }
    } catch (error) {
  ...
  const handleDeactivateAgent = async (agentKey: string) => {
    try {
      const result = await deactivateBenAgent(agentKey);
      if (result.success) {
        alert(`✅ Agent deactivated: ${result.message}`);
      } else {
        alert(`❌ Error: ${result.error}`);
      }
    } catch (error) {
      alert(`❌ Failed to deactivate agent: ${error}`);
    }
  };
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-rensto-bg-primary via-rensto-bg-secondary to-rensto-bg-primary flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <p className="text-rensto-text/70">Loading your Tax4Us portal...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-rensto-bg-primary via-rensto-bg-secondary to-rensto-bg-primary flex items-center justify-center">
        <GlassCard className="p-8 text-center">
          <RenstoStatusIndicator status="error" size="lg" />
          <h2 className="text-2xl font-bold text-rensto-text mt-4">Portal Error</h2>
          <p className="text-rensto-text/70 mt-2">{error}</p>
        </GlassCard>
      </div>
    );
  }
        ...
        <div className="max-w-7xl mx-auto px-4 py-6">
          <div className="flex items-center justify-between">
            <div ref={logoRef} className="flex items-center space-x-4">
              ...
              <div>
                <GradientText className="text-2xl">Tax4Us Portal</GradientText>
                <ShinyText className="text-sm text-rensto-text/70">Ben Ginati - Tax Services</ShinyText>
              </div>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div ref={titleRef} className="text-center mb-8">
          ...
          <ShinyText className="text-xl text-rensto-text/70">
            Your Tax4Us automation agents are ready to work
          </ShinyText>
        </div>
        ...
        <div ref={tabsRef}>
          <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
            <TabsList className="grid w-full grid-cols-6 bg-rensto-card/50 backdrop-blur-sm border border-rensto-border/20">
              ...
              <TabsTrigger value="workflows" className="rensto-tab">
                  ...
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-2xl font-bold text-rensto-text">{agents?.filter(a => a.status === 'active').length || 0}</p>
                      </div>
                  ...
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-2xl font-bold text-rensto-text">42h</p>
                      </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <GlassCard>
                  ...
                  <CardContent className="space-y-4">
                    {[
                      { action: 'Tax return processed', time: '2 min ago', status: 'success' },
                      { action: 'Client data updated', time: '15 min ago', status: 'success' },
                  ...
                  <GlassCard key={agent._id}>
                    ...
                    <CardContent className="space-y-4">
                      <div className="flex items-center justify-between text-sm">
                        ...
                        <span className="text-rensto-text">{agent.lastRun || 'Never'}</span>
                      </div>
                      <div className="flex items-center justify-between text-sm">
                        <span className="text-rensto-text/70">Success Rate:</span>
                        <span className="text-rensto-text">{agent.successRate || '0'}%</span>
                      </div>
                        ...
                        {agent.status === 'active' ? (
                          <>
                            <GradientButton 
                              ...
                              disabled={runningAgents.has(agent._id)}
                            >
                              {runningAgents.has(agent._id) ? (
            ...
            <TabsContent value="workflows" className="space-y-6">
              <GlassCard>
                <CardHeader>
                  <CardTitle className="text-rensto-text">Workflow Status</CardTitle>
                </CardHeader>
                <CardContent>
                  <p className="text-rensto-text/70">Workflow management coming soon...</p>
                </CardContent>
              </GlassCard>
            </TabsContent>
